...
__export(utils_exports, {
  LRUCache: () => LRUCache,
  alreadyCachedDownstream: () => alreadyCachedDownstream,
...
function getFileStats(path2) {
  return fs || getBuiltinModule(), fs.stat(path2).catch(() => null);
}
class LRUCache {
...
function alreadyCachedDownstream(headers, etag, fileStats) {
  ...
  if (headers["if-modified-since"]) {
    ...
    try {
      return fileStats.mtime !== void 0 && fileStats.mtime.getTime() <= Date.parse(ifModifiedSince);
    } catch {
    }
  }
  return !1;
...
function getFile(path2) {
  return (0, import_elysia.file)(path2);
}
async function generateETag(file) {
  if (isBun)
    return new Bun.CryptoHasher("md5").update(await file.value.arrayBuffer()).digest("base64");
  if (Crypto || (Crypto = process.getBuiltinModule("crypto")), !Crypto)

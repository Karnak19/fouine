...
test("delivers a typed event to subscribers", () => {
  ...
  publishEvent({ type: "repo:updated", repo: "a/b", row: { full_name: "a/b", installation_id: 1, prompt: null, model: null, enabled: 1, deny_test_commands: null, auto_merge: null, merge_method: null, created_at: 0 } });
  unsub();
...
test("upsertRepoAndPublish keeps settings an operator already set", () => {
  ...
  repos.update.run({ $full_name: "keep/settings", $prompt: "custom", $model: "m", $enabled: 1, $deny_test_commands: null, $auto_merge: null, $merge_method: null });

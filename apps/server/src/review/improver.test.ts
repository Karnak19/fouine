...
function seedRepo(enabled = 1) {
  ...
  repos.update.run({ $full_name: FULL, $prompt: null, $model: null, $enabled: enabled, $deny_test_commands: null, $auto_merge: null, $merge_method: null });
}
...
test("improver gating: failed run does not advance the marker", async () => {
  ...
  repos.upsert.run({ $full_name: full, $installation_id: 5, $prompt: null, $model: null });
  repos.update.run({ $full_name: full, $prompt: null, $model: null, $enabled: 1, $deny_test_commands: null, $auto_merge: null, $merge_method: null });
  const row = reviews.insert.get({
    $repo: full,
    $pr: 1,
    $title: "t",

...
// Call `${internalUrl()}/internal/sessions/:sid/:path`. The session id is the
// boundary (see server/internal.ts): it resolves to the review row that owns
// owner/repo/PR/kind. Non-2xx responses carry `{ error }`; surface that verbatim
// so the model sees why it failed.

...
| `OPENCODE_BASE_URL` | no | — | **Sidecar mode.** Base URL of an existing OpenCode server (e.g. `http://opencode:4096`). Set = fouine spawns no `opencode` child; unset = child mode (default) |
| `OPENCODE_SERVER_PASSWORD` | no | — | HTTP Basic password for the OpenCode server (username is always `opencode`). Generated by fouine in child mode; in sidecar mode set the *same* value on fouine and the sidecar. Unset = unsecured |
 (OpenCode v1 → v2 migration: singleton server, loopback-proxy GitHub, plugin API)
